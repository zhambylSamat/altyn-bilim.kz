<?php
	...
	try {
		$stmt = $conn->prepare("SELECT s.subject_num subject_num, s.subject_name subject_name, gi.group_name group_name, tp.topic_name topic_name FROM subject s, group_info gi, topic tp WHERE gi.group_info_num = :group_info_num AND s.subject_num = gi.subject_num AND tp.topic_num = :topic_num");
		...
		$sql = "SELECT q.quiz_num quiz_num, qm.quiz_mark_num quiz_mark_num, s.student_num student_num, gs.group_student_num group_student_num, s.name name, s.surname surname, qm.mark_theory mark_theory, qm.mark_practice mark_practice
					FROM group_info gi
					...
					?>
						<!-- <td> -->
							<div class='info-mark'>
								...
								<?php if((isset($result_list[$total_count+1]) && $result_list[$total_count+1]['student_num']!=$student_num) || (isset($result_list[$total_count+1]) && $result_list[$total_count+1]['student_num']==$student_num && $result_list[$total_count+1]['mark_theory']==0 && $result_list[$total_count+1]['mark_practice']==0) || (!isset($result_list[$total_count+1]))) {  ?><a class='btn btn-xs btn-default pull-right edit-btn'>Өзгерту</a>
							</div>
							<form method='post' action='teacher_controller.php' class='edit-mark' style='display: none;'>
								<span class='form-inline'>
									<label>Теория: </label>
