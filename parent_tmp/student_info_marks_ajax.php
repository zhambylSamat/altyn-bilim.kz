	...
	<div class='col-md-6 col-sm-6 col-xs-12'>
		<table class='table table-bordered table-striped'>
			...
			<?php 
				...
				if($tt_result!=''){
					...
					foreach($tt_result as $key => $value){
						// $tmp_arr = array("year"=>substr($value['date'],4,4), "month"=>substr($value['date'],2,2), "day"=>substr($value['date'],0,2), "mark"=>$value['mark']);
						// echo "<script>console.log(new Date(".substr($value['date'],4,4).", ".substr($value['date'],2,2).", ".substr($value['date'],0,2).")); </script>";
						// $tmp_arr = array("x"=>'', "y"=>"parseInt(".$value['mark'].");");
						// array_push($chart_arr, $tmp_arr);
			...
			<?php 
					...
					// $chart_str = json_encode($chart_arr); 
				}
			?>
		</table>
	</div>
