	...
	try {
		...
		$stmt = $conn->prepare("SELECT pg.created_date created_date, 
								...
								WHERE pg.group_info_num in ($group_info_nums)
									-- AND DATE_FORMAT(pg.created_date, '%m-%Y') > DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 12 month), '%m-%Y')
		...
		$month_txt = array("","Қаңтар","Ақпан","Наурыз","Сәуір","Мамыр","Маусым","Шілде","Тамыз","Қыркүйек","Қазан","Қараша","Желтоқсан");
		...
		foreach ($trial_test_sql_result as $key => $value) {
			if (!isset($tt_result[$value['trial_test_mark_num']])) {
				$tt_result[$value['trial_test_mark_num']] = array();
			}
...
<div class='row'>
		...
		<table class='table table-bordered table-striped'>
			...
			<?php 
				...
				if($tt_result!=''){
					...
					$chart_labels = array();
					$chart_datas = array();
					$chart_label = "Сынақ тест: ".$group_info['subject_name'];
			...
			<tr>
				...
				<th>
					...
					<center><?php echo substr($value['date'],2,2).".".substr($value['date'],0,2).".".substr($value['date'],4,4);?></center>
				</th>
			</tr>
			<?php 
					} 
	...
	<div class='col-md-6 col-sm-6 col-xs-12'>
		<div id="chart" style="height: 370px; width: 100%;">
			<canvas id="chartContainer" width="400" height="300"></canvas>
		</div>
	</div>
</div>
