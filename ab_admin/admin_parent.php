...
<?php 
	try {
		$stmt = $conn->prepare("SELECT s.surname, s.name FROM student s where s.student_num not in (select p.student_num from parent p) AND s.student_num != 'US5985cba14b8d3100168809' AND block != 6");
		$stmt->execute();
