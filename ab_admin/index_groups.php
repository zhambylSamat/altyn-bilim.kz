...
<?php
	...
	if(!isset($_GET['search']) || $_GET['search']==''){ 
		try {
		    ...
		    for ($i = 0; $i<count($result_group_info); $i++) {
		    	if(array_key_exists($result_group_info[$i]['group_info_num'], $result_group_schedule)){

		    		$week_txt = "";
		    		foreach ($result_group_schedule[$result_group_info[$i]['group_info_num']] as $key => $value) {
		    			$week_txt .= $week[$value].", ";
		    		}
		    		$week_txt = substr($week_txt, 0, strlen($week_txt)-2);
		    		$result_group_info[$i]["schedule"] = $week_txt;
		    		array_push($result_group_info[$i], $week_txt);
		    	}
		    	else{
		    		$result_group_info[$i]["schedule"] = "N/A";
		    		array_push($result_group_info[$i], "N/A");
		    	}
		    }
		...
		<td style='border-color:black; padding:0;'>
			<div class='group-info'>
				<table class='table table-bordered' style='background-color: <?php echo $bg_color;?>;'>
					...
					<tr>
						...
						<td>
							<center>
								<b>Сабақ кестесі:<br></b> <?php echo $value['schedule']."<br>".$value['start_lesson']."-".$value['finish_lesson'];?>
							</center>
						</td>
					</tr>
				</table>
			...
			<form class="form-inline group-form" onsubmit="return confirm('Подтвердите действие!!!');" action='admin_controller.php' method='post' style='display: none;'>
				...
				<div class='form-group'>
					<?php
						try {
							$stmt = $conn->prepare("SELECT * FROM teacher where block = 0 order by surname asc");
					...
					?>
					<input type="text" class='form-control' disabled value='<?php echo $value['surname']." ".$value['name']; ?>'>
				...
				<div class='form-group'>
					...
					?>
					<!-- <textarea name='group_comment' class='form-control' rows='1' cols='30'></textarea> -->
