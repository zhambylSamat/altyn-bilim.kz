...
<?php 
	...
	$list = array();
	try {
		$stmt = $conn->prepare("SELECT s.subject_num sNum, 
									s.subject_name sName, 
									t.topic_num tNum, 
									t.topic_name tName, 
									st.subtopic_num stNum, 
									st.subtopic_name stName 
								FROM subject s, 
									topic t, 
									subtopic st 
								WHERE s.subject_num = t.subject_num 
									AND t.topic_num = st.topic_num 
								ORDER BY s.created_date, t.created_date, st.created_date ASC");
	    $stmt->execute();
