...
<?php
	if(isset($_GET[md5('subjectNum')]) && strlen($_GET[md5('subjectNum')])>=20){
		try {
			...
			$subject_num = $_GET[md5('subjectNum')];
	     	
		    $stmt->execute();
		    $exist_subject = $stmt->rowCount();
		    if($exist_subject==1){
		    	$subject = $stmt->fetch(PDO::FETCH_ASSOC);
		    	// echo $result_subject['subject_name'];
			}
			else{
				header('location:index.php');
				// echo "error".$exist_subject." ".$subject_num;
			}
...
<body onload="startAjax('<?php echo $subject['subject_num'];?>','ajax_adminTestMain.php')">
	<section>
		<div class="container">
			<div class='row'>
				<div class='col-md-12 col-sm-12'>
					<h3 class='text-primary' id='header-nav'><?php echo $subject['subject_name']." / ";?></h3>
				</div>
			</div>
			<div class='row'>
				<div class='col-md-3 col-sm-3'>
					<div class='btn-group-vertical' style='width:100%;'>
						...
						<button type='button' class='btn btn-primary section' data_name='main_section' data_num='<?php echo $subject['subject_num'];?>'><?php echo $subject['subject_name'];?></button>
						<?php 
							try {
								$stmt = $conn->prepare("SELECT * FROM topic WHERE subject_num = :subject_num");
							    ...
							    $stmt->execute();
							    $result_topic_list = $stmt->fetchAll();
							} catch (PDOException $e) {
								echo "Error ".$e->getMessge()." !!!";
							}
							foreach($result_topic_list as $readrow_topic){
						?>
						<div class='btn-group' role='group' target_name='<?php echo $readrow_topic['topic_name'];?>'>
							<button id='btn-dropdown-1' type='button' class='btn btn-primary dropdown-toggle' data-toggle='dropdown' aria-haspopup='true' aria-expanded='true'>
								<?php echo $readrow_topic['topic_name'];?>
							...
							<ul class='dropdown-menu' aria-labelledby='btn-dropdown-1' style="width:100%;">
								<?php 
									try {
										...
										$topic_num = $readrow_topic['topic_num'];
									    $stmt->execute();
									    $result_subtopic_list = $stmt->fetchAll();
									} catch (PDOException $e) {
										echo "Error ".$e->getMessge()." !!!";
									}
									foreach($result_subtopic_list as $readrow_subtopic){
								?>
								<li>
									<a target_name='<?php echo $readrow_subtopic['subtopic_name'];?>' data_name='subtopic' data_num='<?php echo $readrow_subtopic['subtopic_num'];?>' class='section' style='cursor:pointer;'><span class='glyphicon glyphicon-triangle-right'></span>&nbsp;<?php echo $readrow_subtopic['subtopic_name'];?></a>
								</li>
								<?php }?>
							</ul>
						</div>
