...
<center><h4>Қызметкерлер</h4></center>
<table class='table table-bordered table-striped'>
	...
	<?php 
		$count = 0;
		foreach ($emp_future_dob as $value) {
			$color = '';
			if($value['coming']==1){
				$coming_count++;
				$color='#5CB85C';
	...
	<tr style='<?php echo "background-color:".$color.";"; ?>'>
		...
		<td><?php echo substr($value['dob'], 0, 5);?></td>
	</tr>
	<?php } ?>
	<?php 
		foreach ($emp_past_dob as $value) {
	?>
	<tr>
		<td style='width: 5%;'><center><?php echo ++$count;?></center></td>
		<td><?php echo $value['surname'];?></td>
...
<hr>
<center><h4>Студенттер</h4></center>
<table class='table table-bordered table-striped'>
	<tr style='position: sticky; top:0px; z-index:10;'>
		<th><center>#</center></th>
		<th>Тегі</th>
