 <?php
	...
	//--------------------- $elementNum and $data_num is test_num

	$number_of_exists_question = 0;

	if(isset($_GET['elementNum']) || $data_num){
		$elementNum = isset($_GET['elementNum']) ? $_GET['elementNum'] : $data_num;
		if($elementNum=='new'){
			try {
				$stmt = $conn->prepare("INSERT INTO test (test_num, subtopic_num, type) VALUES(:test_num, :test_num, 'entrance_examination_test') ");

				$stmt->bindParam(':test_num', $elementNum, PDO::PARAM_STR);

			    $elementNum = str_replace('.','',uniqid('TT', true));

			    $stmt->execute();

			    header("location:index.php?data_num=".$elementNum);
...
<div class='test_area'>
	<?php
		$result_question = '';
		try {
			$stmt = $conn->prepare("SELECT q.question_num, 
										q.question_text, 
										q.question_img 
									FROM question q, 
										test t 
									WHERE t.test_num = :test_num 
										AND q.test_num = t.test_num 
		...
		<form method='post' class='add-test-form' enctype="multipart/form-data"> <!-- ajaxDbTest.php -->
			<div class='row'>
				<div class='form-group col-md-12 col-sm-12'>
					...
					<textarea class='form-control' rows="6" name="question-txt" id='question' placeholder="'Сұрақты' белгілеңіз"><?php echo $readrow_question['question_text'];?></textarea>
					<br>
					<br>
