...
if(isset($_GET[md5(md5('new_question'))])){
	...
	try {
		...
		if($data_name=='student'){
			$stmt = $conn->prepare("UPDATE student SET block = 6 WHERE student_num = :student_num");
			$stmt->bindParam(':student_num', $data_num, PDO::PARAM_STR);
			$stmt->execute();

			$stmt = $conn->prepare('UPDATE group_student SET block = 6, block_date = :block_date WHERE student_num = :student_num');
			$stmt->bindParam(':student_num', $data_num, PDO::PARAM_STR);
			$stmt->bindParam(':block_date', $curr_date, PDO::PARAM_STR);
			$stmt->execute();
		}
		else if($data_name=='teacher'){
			$stmt = $conn->prepare("UPDATE teacher SET block = 6, block_date = :block_date WHERE teacher_num = :teacher_num");
			$stmt->bindParam(':teacher_num', $data_num, PDO::PARAM_STR);
			$stmt->bindParam(':block_date', $curr_date, PDO::PARAM_STR);
			$stmt->execute();	
		}
		else if($data_name=='student_group'){
			$stmt = $conn->prepare("UPDATE group_student SET block = 6, block_date = :block_date WHERE group_student_num = :group_student_num");
			...
			$stmt = $conn->prepare("UPDATE group_info SET block = 6 , block_date = :block_date WHERE group_info_num = :group_info_num");
			$stmt->bindParam(':group_info_num', $data_num, PDO::PARAM_STR);
			$stmt->bindParam(':block_date', $curr_date, PDO::PARAM_STR);
			$stmt->execute();

			$stmt = $conn->prepare("UPDATE group_student SET block = 6, block_date = :block_date WHERE group_info_num = :group_info_num");
			$stmt->bindParam(':group_info_num', $data_num, PDO::PARAM_STR);
			$stmt->bindParam(':block_date', $curr_date, PDO::PARAM_STR);
			$stmt->execute();	
		}
