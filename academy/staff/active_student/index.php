<?php
	...
	include_once($root.'/staff/student/active_student/components/add_new_student.php');
?>
			...
			<div class='row' style='padding: 0px 20px;'>
				<div class='short_info std-info'>
					<div class='col-md-6 col-sm-6 col-xs-12'>
						<span class='label label-warning reserve-count'><?php echo count($value['reserves']) > 0 ? count($value['reserves']) : ''; ?></span>
						&nbsp;
						<span class='label label-danger payment_count'><?php echo $value['no_payment_count'] > 0 ? $value['no_payment_count'] : ''; ?></span>
						&nbsp;&nbsp;
						<a href='<?php echo $ab_root."/academy/staff/student/student_cabinet?student_id=".$student_id; ?>' target='_blank' class='cursor-pointer student-short-info'><?php echo $value['full_name']." <span class='phone-number'>".$value['phone']; ?></span>
						</a>
						<button class='btn btn-xs btn-info copy-phone-number-btn'><i class="fas fa-copy"></i></button>
						<?php echo ($value['is_block'] == 1 ? '<p><b>Блокқа түсіп қалды</b></p>' : ''); ?>
								...
								<ol style='padding: 0;'>
									...
									<li style='border: 1px solid lightgray; padding: 5px 0px 5px 10px; list-style-position: inside;'>
										<span data-id='<?php echo $group_id; ?>'>
											...
											| <?php echo $val['subtopic_title']; ?> | <?php echo $val['is_army_group'] ? 'Армия' : ""; ?>
										</span>
										<?php
											$html = '';
											$price = $val['is_army_group'] ? $one_army_lesson_price : $one_lesson_price;
											if ($val['status'] == 'inactive') {
													...
													$html .= "<div class='partial_payment'>";
														...
														$html .= "<form class='form-inline partial_payment_form' style='display:none;'>";
															$html .= "<div class='form-group'>";
																...
																$is_army_group = $val['is_army_group'] ? 1 : 0;
																$html .= "<input type='number' class='partial_payment_days form-control' data-price='".$price."' name='partial_days' min='".$min_days."' step='1' value='".$default_days."'>";
															$html .= "</div>";
															...
															$html .= "<span class='partial_payment_price'> ".($default_days*$price)." тг. </span>";
												...
												$access_until = $val['payment']['access_until'];
												// $is_army_group = $val['is_army_group'] ? 1 : 0;
												// $price = $val['is_army_group'] ? $one_army_lesson_price : $one_lesson_price;
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		if ($student_phone !== undefined) {
			$('input[name=search_student]').val($student_phone);
			filter_student_list($student_phone)
		}
	});
</script>