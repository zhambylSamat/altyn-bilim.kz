<?php
	...
	$tutorial_video_config = false;
	if (isset($subtopic_id) && $subtopic_id != '') {
		$videos = get_tutorial_video($subtopic_id);
		$perceive = get_material_config($subtopic_id, 'tutorial_video');
	}
?>
...
<div class='row'>
	<div class='col-md-12 col-sm-12 col-xs-12'>
		<div class='perceive'>
			...
			<a class='btn btn-xs btn-success material-config-btn hide' data-type='tutorial-video' data-subtopic-id='<?php echo $subtopic_id; ?>'>Сохранить</a>
		</div>
	</div>
	<div class='col-md-6 col-sm-6 col-xs-12'>
		<?php include_once($root.'/staff/material/loads/common/order_action_btns.php'); ?>
		<ul class='list-position-inside list-btn sortable' data-obj='tutorial_video' data-dir='<?php echo $_SERVER['REQUEST_URI']; ?>'>
			<?php
				$html = "";
				foreach ($videos as $value) {
	    			...
	    			if ($value['pop_up'] == 0) {
	    				$pop_up_text = "pop up off";
	 					$pop_up_class = "btn-default pop-up-on";
	    			} else {
	    				$pop_up_text = "pop up on";
	 					$pop_up_class = "btn-success pop-up-off";
	    			}

	    			$pop_up_btn = "<button style='margin-left: 1%;' class='btn btn-sm pop-up-btn pull-right ".$pop_up_class."' data-sid='".$subtopic_id."'>".$pop_up_text."</button>";

					$html .= "<li class='ls-btn show-video' data-id='".$value['id']."' data-order='".$value['video_order']."' data-link='".$value['link']."' data-title='".$value['title']."'>".$value['title']." ".$pop_up_btn." <span class='pull-right'>".$duration."</span></li>";
				}
				echo $html;
			?>
		</ul>
