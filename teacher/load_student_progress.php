	...
	<table class='table table-bordered table-progress'>
		...
		<?php
			...
			foreach($value_topic as $key_st => $value_st){ 
				$last_st_progress = $value_st['progress'];
				$count2++;
		?>
		...
		<?php
			$st_count++;
			if (count($value_topic) == $st_count && $last_st_progress == 0.5) {
				$extra_topic_num = $id;
			}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if("<?php echo $topic_num; ?>"!=''){
			$("tr[data-name=<?php echo $topic_num; ?>]").toggle('fast');
		}
		if ("<?php echo $extra_topic_num; ?>"!='') {
			$("tr[data-name=<?php echo $extra_topic_num; ?>]").toggle('fast');
		}
	});
</script>