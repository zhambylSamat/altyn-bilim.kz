<?php
	...
	try {
		$stmt = $conn->prepare("SELECT s.student_num,
								    ...
								    DATE_FORMAT(ttm.date_of_test, '%Y-%m-%d') AS date_of_test_max,
								    gi.subject_num
								FROM group_info gi
		...
		foreach ($trial_test_sql_result as $key => $value) {
			...
			if ($student_num!=$value['student_num']) {
				...
				$tt_result[$value['student_num']]['date_of_test_max'] = $value['date_of_test_max'];
			}
		...
		<td>
			...
			<form method='post' id='trial_test_form' style='display: none;'>
				<div class='form-group'>
					...
					<!-- datePicke -->
					<input type="date" class='form-control' placeholder="dd.mm.yyyy" id='trial-date' min="<?php echo $value['date_of_test_max']; ?>" name="trial_date" required="" style='display:inline-block; width: 50%;'>
				</div>
				<div class='form-group'>
