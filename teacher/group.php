...
<div id='group-body'>
	<div class='container'>
		<div class='row'>
			<div class='col-md-12 col-sm-12'>
				<?php if($as_admin==$true){ ?><h5 style='color:orange;'>* Вы вошли в страницу как администратор.</h5><?php } ?>
				<ul class="nav nav-tabs">
				 	<li role="presentation" class='navigation active' data='mark'><a href="#">Баға қою</a></li>
				 	<?php if($as_admin == $false){ ?>
				 	<li role="presentation" class="navigation" data='students'><a href="#">Студенттер</a></li>
				 	<?php }?>
				</ul>
				<br>
			</div>
	...
	<div class='container'>
		<div class='row'>
			<div class='col-md-12 col-sm-12'>
				<div class='mark box' style='background-color: white;'>
					<center>
					...
					<?php
						$month = array("","Қаңтар","Ақпан","Наурыз","Сәуір","Мамыр","Маусым","Шілде","Тамыз","Қыркүйек","Қазан","Қараша","Желтоқсан");
						try {
					...
					<div class='outer'>
						<div class='inner marks' style='border-right: 2px solid gray;'>
							<?php
								try {
					...
					<hr>
					<div class='quiz' style='margin-top:110px;'>
						<div class='row'>
	...
	<div class='container'>
		<div class='row'>
			<div class='col-md-12 col-sm-12'>
				<div class='students box' style='overflow-x: scroll; display: none;'>
					<?php
						$group_info_num = $_GET['data_num'];
						include_once('getStudentsByGroupNum.php');
					?>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	$(document).on('click','.set-comment',function(){
		...
		$('.box-comment-for-teacher .modal-body').load('load_comment.php?gsn='+$gsn+"&data_num=<?php echo $_GET['data_num'];?>&id="+$id+"&sj_num=<?php echo $subject_num;?>");
	});
	// ----------------------------review-end----------------------------------
	...
	$(document).on('click','.header-student',function(){
		...
		$subject_num = $(this).attr('subject_num');
	...
	$(document).on('submit','#trial_test_form',(function(e) {
		console.log('trial_test_form clicked');
		$thisParent = $(this);
		e.preventDefault();
		$.ajax({
	...
	function check_home_work() {
		$mark_serializer = $('.adding_mark').serialize();
		$mark_serializer_arr = $mark_serializer.split('&');
		$is_attendance = false;
		$is_not_home_work = false;
		$.each($mark_serializer_arr, function(index, value){
			$key_val = value.split('=');
			if ($key_val[0].indexOf("attendance") >= 0 && $key_val[1] == 1){
				$is_attendance = true;
			} else if ($is_attendance) {
				$is_attendance = false;
				if ($key_val[0].indexOf("home_work") >= 0 && $key_val[1] == 0) {
					$is_not_home_work = true;
				}

			}
		});
		if ($is_not_home_work) {
			if (confirm('Оқушы(ларға) уй жұмысына 0 қойылды')) {
				return true;
			}
			return false;
		}
