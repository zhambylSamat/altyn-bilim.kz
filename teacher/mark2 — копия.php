<?php
	...
	try {
		...
		$edit_datas = isset($_GET['edit']) ? $_GET['edit'] : false; 
		...
		<tr>
			...
			<?php
				...
				if(empty($result_date)){
					$number = cal_days_in_month(CAL_GREGORIAN, date($date_number), date("Y"));
					$min_day = 0;
		...
		?>
			<td class='not-fix' data='<?php echo $count;?>' style='border-left:2px solid #999;'>
				...
				<?php 
					...
					if(true){
				?>
				<div class='<?php echo (!$new_marks) ? 'new-data' : 'last-data' ?>' style='<?php echo (!$new_marks) ? 'display: none;' : '' ;?>'>
					<center>
