<?php
	...
	$search_order_type = isset($search_order_type) && $search_order_type != '' ? $search_order_type : (isset($_GET['search_order_type']) ? $_GET['search_order_type'] : $avg_practice);
	try {
		$result_stmt = array();
		if($search_order_type == $avg_practice){
			$stmt = $conn->prepare("SELECT sj.subject_name,
									    ...
									    FORMAT((SELECT AVG(qm2.mark_practice)
									    FROM quiz_mark qm2,
									     	quiz q2,
									     	topic t2
									    WHERE qm2.quiz_num = q2.quiz_num
									    	AND q2.topic_num = t2.topic_num
									   		AND t2.subject_num = sj.subject_num
									    	AND qm2.student_num = s.student_num), 2) as order_type
									FROM quiz_mark qm,
										student s,
									    quiz q,
									    subject sj,
		...
		else if($search_order_type == $avg_theory){
			$stmt = $conn->prepare("SELECT sj.subject_name,
										sj.subject_num,
										t.topic_num,
										t.topic_name,
									    ...
									    WHERE qm2.quiz_num = q2.quiz_num
									    	AND q2.topic_num = t2.topic_num
									   		AND t2.subject_num = sj.subject_num
									    	AND qm2.student_num = s.student_num) as order_type_count,
									    FORMAT((SELECT AVG(qm2.mark_theory)
									    FROM quiz_mark qm2,
									     	quiz q2,
									     	topic t2
									    WHERE qm2.quiz_num = q2.quiz_num
...
?>
	...
	<table class='table table-bordered'>
		<tr>
			...
			<?php
				if($search_order_type == $avg_practice){
					echo "Есеп. Орташа балл";
				}
				else if($search_order_type == $avg_theory){
					echo "Теория Орташа балл";
				}
				else if($search_order_type == $last_result_practice){
